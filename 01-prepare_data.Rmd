...
We provide a built-in dataset that can be used to explore VCFshiny. The data set can be directly loaded into the APP by clicking the button *“Use example data?”* of the data input module (Figure S4-II). The built-in dataset was derived from the sequencing data of published articles, including a control group and three experimental groups, with three replicates in each group ([Liang Y, Xie J, Zhang Q, et al. 2022](references.html#ref13)). The sequencing data was first mapping to the reference, followed by gatk Variants calling and Annovar annotations.
