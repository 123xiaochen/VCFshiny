...
### Module 3: Venn Diagram
  ...
  In order to detect the duplicate data credibility of each group, users can cross-analyze the data using the Venn R package ([Dusa, Adrian. 2021](references.html#ref14)) and present the visualized results.
...
### Module 7: Circos Plot
  ...
  In order to observe the distribution of SNP or Indel on chromosomes, the “Circos Plot” module uses the circlize R package ([Gu, Z.et al. 2014](references.html#ref15)) to plot the selected data on a chromosomal circle plot according to the location of mutations. 
...
### Module 11: Cancer Signature

Mutational signals are the result of a mutational process consisting of some form of DNA damage and subsequently acted upon by DNA repair or replication mechanisms. These mutational signals reveal both endogenous and exogenous factors in the development of cancer. VCFshiny provides the analysis of mutant characteristics by software package Musicatk ([Chevalier, et al., 2021](references.html#ref10)), which is a comprehensive mutant signature analysis kit for discovery, prediction and analysis.

This moudle includes three parts: This module consists of three parts. (1) The first part is data selection and analysis parameter setting. Users can choose the data they are interested in for self-defined analysis, and the analysis libraries they are interested in include SBS (SBS96), DBS (DBS78) and Indel (IND83) for analysis *(Figure 3.15-I)*. (2) The second part is the visualization of the data. After completing the corresponding analysis, the analysis results are visualized, and user can use statistics to download the visual results *(Figure 3.15-II)*. (3) The third part is the data display part, and the data can also be downloaded *(Figure 3.15-III)*.

```{r echo = F, fig.cap = "Cancer_Signature", out.width="100%",fig.align='center'}
knitr::include_graphics("figure/11.Cancer_Signature.png")
``` 

## Part 2: Annotation Part

VCFshiny not only provides analysis of mutated data after annotation, but also provides user friendly annotation features. VCFshiny provides two kinds of software, *Annovar* and *VariantAnnotation*, to annotate  human variant VCF files. After downloading the annotation results, they can be uploaded directly to the analysis part for analysis.

### Module 1: Annovar Annotation

ANNOVAR ([Wang K, Li M, Hakonarson H. 2010](references.html#ref11)) is an efficient software tool for functional annotation of genetic variation detected in the genome using up-to-date information. Given a list of chromosomal variants, starting positions, ending positions, reference nucleotides, and observed nucleotides, ANNOVAR can perform annotation in three ways: 
...
VariantAnnotation ([Obenchain, V., et al. 2014](reference.html#12)) is an *R / Bioconductor* package for the exploration and annotation of genetic variants. Capabilities exist  for reading, writing and filtering variant call format (VCF) files.

The second Annotation tool provided by VCFshiny for variable Data is VariantAnnotation, which can be annotated by users in the "VariantAnnotation Annotation" module on the "Data Annotation" page *(Figure 3.17-I)*, which includes two parts: (1) The first part is the reading of data and the selection of annotation parameters. Users can upload their own variation data and select annotation parameters for annotation *(Figure 3.17-II)*. (2) The second part is the display part of annotation results. After data upload and annotation, annotation results will be displayed in the form of a table in the second part. At the same time, the built-in download button enables users to download annotation results for further analysis *(Figure 3.17-III)*.
...
Before using Annovar annotation in VCFshiny, required annotation Database needs to be downloaded. The "Download Annovar Database" module in the "Data Annotation" page of VCFshiny provides a convenient download page *(Figure 3.18-I)*. Users can download the database in Annovar software according to their own needs. This page includes three parts: (1) Download parameter selection section. Users can select the type and name of the database to download according to their needs *(Figure 3.18-II)*. (2) Download the display part of the database, after the completion of the database download, the current downloaded database is displayed *(Figure 3.18-III)*. (3) The database name provided by Annovar. We will display the downloaded data provided by Annovar, and users can download the database according to the display information *(Figure 3.18-IV)*.
